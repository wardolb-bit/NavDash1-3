    ...
    <main className="min-h-screen bg-[#04080c] p-2 text-slate-100">
      ...
      <div className="mb-2 grid gap-2 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <aside className="space-y-2">
          ...
          <section className="border border-slate-700/50 bg-[#071019] p-3"><div className="flex items-center justify-between"><div className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-500">MAX SEAS ALONG VOYAGE</div><span className="text-[9px] font-black text-cyan-300">WW3</span></div><div className="mt-2 text-4xl font-black text-[#f1d56b]">{maxSeas?.waveHeightFt == null ? "NO WAVE DATA" : `${maxSeas.waveHeightFt.toFixed(1)} ft`}</div>{maxSeas?.wavePeriodSec != null && <div className="mt-1 text-sm text-slate-300">{maxSeas.wavePeriodSec.toFixed(0)} s • {compass(maxSeas.waveDirectionDeg)}</div>}<div className="mt-2 text-[10px] font-bold text-slate-400">{occurrence(maxSeas) || "No route wave sample available."}</div></section>
          <section className="border border-slate-700/50 bg-[#071019] p-3"><div className="flex items-center justify-between"><div className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-500">MAX WIND ALONG VOYAGE</div><span className="text-[9px] font-black text-cyan-300">NOAA</span></div><div className="mt-2 text-4xl font-black text-cyan-300">{maxWind?.windKt == null ? "NO WIND DATA" : `${maxWind.windKt.toFixed(0)} kt`}</div>{maxWind?.windDirectionDeg != null && <div className="mt-1 text-sm text-slate-300">{compass(maxWind.windDirectionDeg)} • GUST {maxGust?.gustKt == null ? "--" : `${maxGust.gustKt.toFixed(0)} kt`}</div>}<div className="mt-2 text-[10px] font-bold text-slate-400">{occurrence(maxWind) || "No route wind sample available."}</div></section>
