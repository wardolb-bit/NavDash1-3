...
type CrewWeather = { ndbc?: { windKt?: number | null; waveFt?: number | null } };
...
function parseAisLine(line: string) { const t = line.trim(); if (!/^[!$]AIVD[MO]/.test(t)) return null; const p = t.split("*")[0].split(","); if (p.length < 7) return null; return { source: t.includes("AIVDO") ? "AIVDO" as const : "AIVDM" as const, total: Number(p[1]), fragment: Number(p[2]), sequence: p[3] || "", channel: p[4] || "", payload: p[5] || "", fillBits: Number(p[6] || 0) }; }
function decodePayload(payload: string, fillBits: number, source: "AIVDO" | "AIVDM") {
  const raw = aisPayloadToBits(payload), bits = fillBits > 0 ? raw.slice(0, -fillBits) : raw; const type = readUnsigned(bits, 0, 6), mmsi = readUnsigned(bits, 8, 30); if (type === null || mmsi === null) return null;
  ...
  if (latRaw === null || lonRaw === null) return { type, source, mmsi }; const lat = latRaw / 600000, lon = lonRaw / 600000; if (Math.abs(lat) > 90 || Math.abs(lon) > 180) return { type, source, mmsi };
...
function decodeAisLine(line: string, fragments: React.MutableRefObject<Map<string, FragmentBuffer>>) {
  const p = parseAisLine(line); if (!p || !p.payload) return null; const now = Date.now(); for (const [k, f] of fragments.current) if (now - f.firstSeen > FRAGMENT_TTL_MS) fragments.current.delete(k);
  if (p.total <= 1) return decodePayload(p.payload, p.fillBits, p.source); const key = `${p.source}-${p.sequence || "no-seq"}-${p.channel}`; const f = fragments.current.get(key) || { total: p.total, parts: [], fillBits: p.fillBits, firstSeen: now }; f.parts[p.fragment - 1] = p.payload; f.fillBits = p.fillBits; fragments.current.set(key, f); if (f.parts.filter(Boolean).length !== f.total) return null; fragments.current.delete(key); return decodePayload(f.parts.join(""), f.fillBits, p.source);
}
function extractLine(message: any) { if (typeof message === "string") return message.trim(); return String(message?.line || message?.sentence || message?.nmea || "").trim(); }
...
export default function CrewViewPage() {
  ...
  useEffect(() => { let closed = false; const ws = new WebSocket(getAisWebSocketUrl()); ws.onopen = () => setConnection("LIVE"); ws.onerror = () => setConnection("ERROR"); ws.onclose = () => { if (!closed) setConnection("OFFLINE"); }; ws.onmessage = (event) => { let parsed: any = event.data; try { parsed = JSON.parse(event.data); } catch {} const line = extractLine(parsed); if (!line) return; const d = decodeAisLine(line, fragments); if (!d?.mmsi || d.source !== "AIVDO") return; setTargets((current) => ({ ...current, [d.mmsi!]: { ...current[d.mmsi!], ...d, lastSeen: Date.now() } as AisTarget })); }; return () => { closed = true; ws.close(); }; }, []);
  useEffect(() => {
    if (ownShip?.lat === undefined || ownShip?.lon === undefined) return;
    let cancelled = false;
    const load = async () => {
      const lat = ownShip.lat!, lon = ownShip.lon!;
      if (route?.waypoints?.length && route.waypoints.length > 1) {
        try {
          const planResponse = await fetch("/api/weather-plan-state", { cache: "no-store" });
          const plan = planResponse.ok ? await planResponse.json() : null;
          const weatherResponse = await fetch("/api/crew-route-weather", { method: "POST", headers: { "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ waypoints: route.waypoints, shipLat: lat, shipLon: lon, departure: plan?.departure ?? null, speedKt: plan?.speedKt ?? null }) });
  ...
  return <main className={`min-h-screen ${page}`}><style jsx global>{`body:has(.crew-view) .navdash-global-nav{display:none!important}.crew-view *{border-radius:0!important}.crew-view .leaflet-control-container *{border-radius:2px!important}`}</style><div className="crew-view mx-auto max-w-[1800px] p-3 sm:p-4">
    <header className={`mb-3 border p-3 ${panel}`}><div className="flex items-center justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-[.18em] text-[#c9a227]">M/V MB480 · NAVDASH 1.3</div><h1 className="mt-1 text-xl font-black uppercase tracking-[.08em] sm:text-2xl">Crew View</h1></div><button type="button" onClick={toggleTheme} className={`shrink-0 border px-3 py-2 text-[10px] font-black uppercase ${ctl}`}>{nightMode ? "Day" : "Night"}</button></div></header>
    ...
    <section className={`border p-4 ${panel}`}><Title text="Voyage" muted={muted} /><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2"><Metric label="Route" value={route?.routeName || "No route loaded"} inset={inset} muted={muted} wide /><Metric label="Active Leg" value={routePreviousWaypoint && routeNextWaypoint ? `${routePreviousWaypoint.name} → ${routeNextWaypoint.name}` : "--"} inset={inset} muted={muted} wide /><Metric label="Next Waypoint" value={routeActiveIndex !== null && routeNextWaypoint ? `${routeNextWaypoint.id} · ${routeNextWaypoint.name}` : "--"} inset={inset} muted={muted} wide /><Metric label="Next WP" value={nav ? `${nav.nextDistance.toFixed(1)} nm` : "--"} inset={inset} muted={muted} /><Metric label="Distance To Go" value={nav ? `${nav.remaining.toFixed(1)} nm` : "--"} inset={inset} muted={muted} /><Metric label="ETA" value={formatEta(etaHours)} inset={inset} muted={muted} /><Metric label="Cross Track" value={nav ? `${nav.xte.toFixed(2)} nm` : "--"} inset={inset} muted={muted} /></div><div className={`mt-3 border p-3 ${inset}`}><div className="mb-2 flex justify-between text-[10px] font-black uppercase tracking-[.12em]"><span className={muted}>Route Progress</span><span>{nav ? `${nav.progress.toFixed(0)}%` : "--"}</span></div><div className={day ? "h-2 bg-slate-200" : "h-2 bg-black/40"}><div className="h-2 bg-[#c9a227]" style={{ width: `${nav?.progress || 0}%` }} /></div></div></section>
...
function Status({ label, value, panel, muted, accent = false }: { label: string; value: string; panel: string; muted: string; accent?: boolean }) { return <div className={`border p-3 ${panel}`}><div className={`text-[9px] font-black uppercase tracking-[.14em] ${muted}`}>{label}</div><div className={`mt-1 font-mono text-lg font-black ${accent ? "text-emerald-500" : ""}`}>{value}</div></div>; }
function Metric({ label, value, inset, muted, wide = false }: { label: string; value: string; inset: string; muted: string; wide?: boolean }) { return <div className={`border p-3 ${inset} ${wide ? "sm:col-span-2 lg:col-span-1 xl:col-span-2" : ""}`}><div className={`text-[9px] font-black uppercase tracking-[.14em] ${muted}`}>{label}</div><div className="mt-1 break-words font-mono text-sm font-black">{value}</div></div>; }
