...
function BridgeSky({stars,center}:{stars:StarSolution[];center:number}){
  ...
  return <svg viewBox={`0 0 ${w} ${h}`} className="w-full" aria-label="Bridge window celestial star view">
    <rect x={left} y={top} width={right-left} height={horizon-top} fill="#020508" stroke="#263442"/>
    <rect x={left} y={horizon} width={right-left} height="52" fill="#071019" stroke="#263442"/>
    ...
    <text x={left+8} y={horizon+18} fill="#e7c95c" fontSize="12" fontWeight="800">HORIZON 0°</text>
    {[-90,-45,0,45,90].map(r=>{const x=left+((r+90)/180)*(right-left);return <g key={r}><line x1={x} x2={x} y1={top} y2={horizon} stroke={r===0?"#526879":"#16202a"} strokeDasharray={r===0?"":"4 8"}/><text x={x} y={505} textAnchor="middle" fill={r===0?"#e7c95c":"#8294a5"} fontSize="13" fontWeight="800">{r===0?"LOOK":r<0?`P ${Math.abs(r)}°`:`S ${r}°`}</text></g>})}
    {shown.map(s=>{const x=left+((s.rel+90)/180)*(right-left),y=horizon-(s.hc/maxAlt)*(horizon-top),bright=s.mag<=1.5;return <g key={s.name}><circle cx={x} cy={y} r={bright?5:3.5} fill={bright?"#e7c95c":"#fca5a5"}/><text x={x+8} y={y-7} fill={bright?"#f3dfa0":"#fecaca"} fontSize="12" fontWeight="800">{s.name}</text><text x={x+8} y={y+8} fill="#708496" fontSize="9">Hc {s.hc.toFixed(0)}° · Zn {s.zn.toFixed(0)}°T</text></g>})}
  </svg>
}
...
function Horizon360({stars}:{stars:StarSolution[]}){
  ...
  return <svg viewBox={`0 0 ${size} ${size}`} className="mx-auto h-auto w-full max-w-[680px]" aria-label="360 degree star horizon plot">
    ...
    <text x={c} y="22" textAnchor="middle" fill="#e7c95c" fontWeight="800">N 000°</text><text x={size-8} y={c+5} textAnchor="end" fill="#e7c95c" fontWeight="800">E 090°</text><text x={c} y={size-8} textAnchor="middle" fill="#e7c95c" fontWeight="800">S 180°</text><text x="8" y={c+5} fill="#e7c95c" fontWeight="800">W 270°</text>
    {shown.map(s=>{const r=((90-s.hc)/90)*radius,a=s.zn*Math.PI/180,x=c+r*Math.sin(a),y=c-r*Math.cos(a),bright=s.mag<=1.5;return <g key={s.name}><circle cx={x} cy={y} r={bright?5:3.5} fill={bright?"#e7c95c":"#fca5a5"}/><text x={x+7} y={y-6} fill={bright?"#f3dfa0":"#fecaca"} fontSize="11" fontWeight="800">{s.name}</text></g>})}
  </svg>
}
...
function LopPlot({sights,apLat,apLon}:{sights:Sight[];apLat:number;apLon:number}){
  ...
  return <div>
    <svg viewBox={`0 0 ${size} ${size}`} className="mx-auto w-full max-w-[680px]" aria-label="Celestial line of position plot">
      <rect x="1" y="1" width={size-2} height={size-2} fill="#020508" stroke="#263442"/>
      ...
      <text x={c+6} y="16" fill="#8294a5" fontSize="10">N</text><text x={size-18} y={c-6} fill="#8294a5" fontSize="10">E</text>
      {lines.map((s,i)=>{const z=s.zn*Math.PI/180,nx=Math.sin(z),ny=Math.cos(z),px=nx*s.intercept,py=ny*s.intercept,dx=Math.cos(z),dy=-Math.sin(z),p1=toPx(px-dx*100,py-dy*100),p2=toPx(px+dx*100,py+dy*100),p=toPx(px,py);return <g key={s.id}><line x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke={i%2===0?"#e7c95c":"#42d3c8"} strokeWidth="2"/><circle cx={p.x} cy={p.y} r="4" fill="#edf4fa"/><text x={p.x+7} y={p.y-7} fill="#aebdca" fontSize="9">{s.body}</text></g>})}
...
export default function BridgeCelestial(){
  ...
  const card="border border-[#263442] bg-[#071019]";
  const label="text-[9px] font-black uppercase tracking-[.16em] text-[#708496]";
  const btn="h-9 border border-[#33485a] bg-[#08121b] px-3 text-[10px] font-black tracking-[.08em] text-[#c7d2dc] hover:border-[#c9a227]/60";
  const active="border-[#c9a227] bg-[#c9a227] text-[#071019]";

  return <main className="min-h-screen bg-[#04080c] text-[#dbe5ee]">
    <style jsx global>{`.navdash-global-nav{display:none!important}`}</style>
    <div className="p-[6px]">
      <header className="mb-[5px] border border-[#c9a227]/30 bg-[#071019]">
        <div className="grid min-h-[46px] grid-cols-1 items-center gap-2 px-3 py-2 xl:grid-cols-[260px_1fr_180px] xl:py-0">
          <div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center border border-[#c9a227] font-black text-[#e7c95c]">N</span><span className="leading-none"><b className="block text-[11px] tracking-[.08em]">NAVDASH</b><small className="mt-1 block text-[8px] tracking-[.14em] text-[#8294a5]">M/V MB480 · CELESTIAL CONSOLE</small></span></div>
      ...
      {mode==="finder"&&<div className="grid gap-[5px] xl:grid-cols-[minmax(0,1fr)_330px]">
        <section className={card}>
          <div className="flex flex-wrap items-center gap-1 border-b border-[#263442] p-2">{viewModes.map(([id,text])=><button key={id} onClick={()=>setView(id)} className={`${btn} ${view===id?active:""}`}>{text}</button>)}</div>
          <div className="p-2">{view==="360"?<Horizon360 stars={stars}/>:<BridgeSky stars={stars} center={center}/>}</div>
        </section>
        <aside className={card}>
          <div className="border-b border-[#263442] p-3"><div className={label}>VIEW</div><div className="mt-1 text-2xl font-black text-[#e7c95c]">{view==="360"?"360° TRUE":`${center.toFixed(0)}° T`}</div></div>
          <div className="grid grid-cols-2">
            <div className="border-b border-r border-[#263442] p-3"><div className={label}>HDG</div><div className="mt-1 text-2xl font-black">{nav.heading?.toFixed(0)??"--"}°</div></div>
            <div className="border-b border-[#263442] p-3"><div className={label}>COG</div><div className="mt-1 text-2xl font-black">{nav.cog?.toFixed(0)??"--"}°</div></div>
          </div>
          <div className="border-b border-[#263442] p-3"><div className={label}>POSITION</div><div className="mt-2 text-sm font-black text-[#42d3c8]">{hideGps?"GPS HIDDEN":<>{formatLatitude(nav.lat)}<br/>{formatLongitude(nav.lon)}</>}</div><button onClick={()=>setHideGps(v=>!v)} className={`${btn} mt-3`}>{hideGps?"SHOW GPS":"HIDE GPS"}</button></div>
          <div className="p-3 text-[10px] leading-5 text-[#8294a5]">Heading is primary orientation. COG is fallback. Rounded star data is for identification and planning.</div>
        </aside>
      </div>}

      {mode==="planner"&&<div className="grid gap-[5px] xl:grid-cols-[380px_minmax(0,1fr)]">
        <aside className={card}>
          <div className="border-b border-[#263442] p-3"><div className={label}>PLANNING UTC</div><input type="datetime-local" value={plannerTime} onChange={e=>setPlannerTime(e.target.value)} className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2 text-sm text-[#dbe5ee]"/></div>
          <div className="border-b border-[#263442] p-3"><div className={label}>BEST 3-STAR SPREAD</div><div className="mt-2 space-y-2">{best3.map(s=><div key={s.name} className="grid grid-cols-[1fr_auto] gap-3 border border-[#263442] bg-[#050a0f] p-2"><b className="text-[#e7c95c]">{s.name}</b><span className="text-xs">{s.zn.toFixed(0)}°T / {s.hc.toFixed(0)}°</span></div>)}</div></div>
          <div className="p-3 text-[10px] leading-5 text-[#8294a5]">Planner favors bright stars between 15° and 65° and maximizes azimuth separation. Polaris is excluded from the 3-star geometry pick but remains available below.</div>
        </aside>
        <section className={card}>
          <div className="grid grid-cols-[1fr_90px_90px_80px] border-b border-[#263442] p-2 text-[9px] font-black tracking-[.12em] text-[#708496]"><span>BODY</span><span>Zn</span><span>Hc</span><span>MAG</span></div>
          <div>{usable.slice(0,20).map(s=><button key={s.name} onClick={()=>{setBody(s.name);setMode("reduction")}} className="grid w-full grid-cols-[1fr_90px_90px_80px] border-b border-[#18222d] p-2 text-left text-xs hover:bg-[#0a151f]"><b className={best3.some(b=>b.name===s.name)?"text-[#e7c95c]":"text-[#dbe5ee]"}>{s.name}{s.name==="Polaris"?" · POLARIS":""}</b><span>{s.zn.toFixed(1)}°</span><span>{s.hc.toFixed(1)}°</span><span>{s.mag.toFixed(1)}</span></button>)}</div>
        </section>
      </div>}

      {mode==="reduction"&&<div className="grid gap-[5px] xl:grid-cols-[420px_minmax(0,1fr)]">
        <aside className={card}>
          <div className="border-b border-[#263442] p-3"><div className={label}>STAR / BODY</div><select value={body} onChange={e=>setBody(e.target.value)} className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2 text-sm">{NAV_STARS.map(s=><option key={s.name}>{s.name}</option>)}</select></div>
          <div className="grid grid-cols-2 border-b border-[#263442]">
            <label className="border-r border-[#263442] p-3"><span className={label}>Hs DEGREES</span><input value={hsDeg} onChange={e=>setHsDeg(e.target.value)} inputMode="decimal" className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2 text-lg font-black"/></label>
            <label className="p-3"><span className={label}>Hs MINUTES</span><input value={hsMin} onChange={e=>setHsMin(e.target.value)} inputMode="decimal" className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2 text-lg font-black"/></label>
          </div>
          <div className="grid grid-cols-2 border-b border-[#263442]">
            <label className="border-r border-[#263442] p-3"><span className={label}>INDEX CORR ' (+/-)</span><input value={ic} onChange={e=>setIc(e.target.value)} inputMode="decimal" className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2"/></label>
            <label className="p-3"><span className={label}>HEIGHT EYE m</span><input value={eye} onChange={e=>setEye(e.target.value)} inputMode="decimal" className="mt-2 w-full border border-[#33485a] bg-[#04080c] p-2"/></label>
          </div>
          <button onClick={addSight} className="m-3 h-10 border border-[#c9a227] bg-[#c9a227] px-4 text-xs font-black text-[#071019]">ADD SIGHT TO PLOT</button>
        </aside>
        <section className={card}>
          <div className="border-b border-[#263442] p-3"><div className={label}>REDUCTION · {selected.name}</div><div className="mt-1 text-sm text-[#8294a5]">UTC {now.toISOString().slice(11,19)} · AP {formatLatitude(nav.lat)} / {formatLongitude(nav.lon)}</div></div>
          <div className="grid sm:grid-cols-2 xl:grid-cols-4">
            {[["Hs",`${reduction.hs.toFixed(4)}°`],["IC",`${reduction.ic.toFixed(2)}'`],["Dip",`${reduction.dip.toFixed(2)}'`],["Refraction",`${reduction.refraction.toFixed(2)}'`],["Ho",`${reduction.ho.toFixed(4)}°`],["Hc",`${selected.hc.toFixed(4)}°`],["Zn",`${selected.zn.toFixed(1)}°T`],["Intercept",`${Math.abs(reduction.intercept).toFixed(1)} NM ${reduction.intercept>=0?"TOWARD":"AWAY"}`]].map(([a,b],i)=><div key={a} className={`p-4 ${i%4<3?"xl:border-r":""} border-b border-[#263442]`}><div className={label}>{a}</div><div className={`mt-2 text-xl font-black ${a==="Intercept"?"text-[#e7c95c]":""}`}>{b}</div></div>)}
          </div>
      ...
      {mode==="plot"&&<div className="grid gap-[5px] xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className={card}>
          <div className="flex items-center justify-between border-b border-[#263442] p-3"><div><div className={label}>SIGHT LOG</div><b className="text-lg">{sights.length} LOP{sights.length===1?"":"s"}</b></div><button onClick={()=>setSights([])} className={btn}>CLEAR</button></div>
          <div>{sights.slice().reverse().map(s=><div key={s.id} className="border-b border-[#18222d] p-3"><div className="flex justify-between gap-2"><b className="text-[#e7c95c]">{s.body}</b><span className="text-xs text-[#8294a5]">{s.time.slice(11,19)}Z</span></div><div className="mt-1 text-xs">Zn {s.zn.toFixed(1)}° · Hc {s.hc.toFixed(3)}° · Ho {s.ho.toFixed(3)}°</div><div className="mt-1 text-xs font-black">{Math.abs(s.intercept).toFixed(1)} NM {s.intercept>=0?"T":"A"}</div></div>)}</div>
          {!sights.length&&<div className="p-4 text-xs text-[#708496]">No sights stored. Reduce a star and tap ADD SIGHT TO PLOT.</div>}
        </aside>
      </div>}

      <footer className="mt-[5px] border border-[#263442] bg-[#071019] px-3 py-2 text-[9px] leading-4 text-[#708496]">CELESTIAL WORKSTATION · Planning and training aid. Do not substitute rounded internal star data or computed output for the current Nautical Almanac, approved publications, required navigation equipment, or the navigator’s independent checks.</footer>
    </div>
  </main>
}
