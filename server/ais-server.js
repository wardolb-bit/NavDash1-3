...
const SHARED_STATE_PATH = process.env.NAV_SHARED_STATE_PATH || path.join(DATA_DIR, 'shared-state.json');
...
const SHARED_STATE_MAX_BYTES = 5 * 1024 * 1024;
...
const httpServer = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/health') {
    ...
    res.end(JSON.stringify({ok:true,websocket:`ws://10.129.4.102:${WS_PORT}`,secureWebsocket:secureServer?`wss://10.129.4.102:${WSS_PORT}`:null,sharedStateEntries:Object.keys(sharedState).length}));
    return;
  }
...
function loadSharedState(){try{if(!fs.existsSync(SHARED_STATE_PATH))return{};const parsed=JSON.parse(fs.readFileSync(SHARED_STATE_PATH,'utf8'));return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:{};}catch(error){console.log(`[STATE] Could not load shared state: ${error.message}`);return{};}}
function saveSharedState(){try{fs.mkdirSync(path.dirname(SHARED_STATE_PATH),{recursive:true});fs.writeFileSync(SHARED_STATE_PATH,JSON.stringify(sharedState,null,2),'utf8');}catch(error){console.log(`[STATE] Could not save shared state: ${error.message}`);}}
function validSharedKey(key){return typeof key==='string'&&key.length>0&&key.length<=512;}
function getSharedState(key){const entry=sharedState[key];if(!entry)return null;const expiresAt=entry.expiresAt?Date.parse(entry.expiresAt):NaN;if(Number.isFinite(expiresAt)&&expiresAt<=Date.now()){delete sharedState[key];saveSharedState();return null;}return entry;}
function putSharedState(key,value,ttlMs,updatedAt){if(!validSharedKey(key))return false;let encoded='';try{encoded=JSON.stringify(value);}catch{return false;}if(Buffer.byteLength(encoded,'utf8')>SHARED_STATE_MAX_BYTES)return false;const now=Date.now(),ttl=Number(ttlMs);sharedState[key]={value,updatedAt:typeof updatedAt==='string'?updatedAt:new Date(now).toISOString(),expiresAt:Number.isFinite(ttl)&&ttl>0?new Date(now+ttl).toISOString():null};saveSharedState();return true;}
function sendSharedResult(ws,key,requestId){const entry=getSharedState(key);ws.send(JSON.stringify({type:'shared-state-result',requestId,key,found:Boolean(entry),value:entry?.value,updatedAt:entry?.updatedAt??null,expiresAt:entry?.expiresAt??null}));}
function mirrorRouteToShared(route){putSharedState('route-state',{hasRoute:Array.isArray(route?.waypoints)&&route.waypoints.length>=2,...route},null,route?.savedAt);}
...
if(lastRouteState)mirrorRouteToShared(lastRouteState);
fs.watchFile(ROUTE_STATE_PATH,{interval:2000},current=>{const existsNow=current.nlink>0;if(existsNow){const disk=loadRouteState();if(disk&&JSON.stringify(disk)!==JSON.stringify(lastRouteState)){lastRouteState=disk;mirrorRouteToShared(lastRouteState);broadcast(lastRouteState);console.log(`[AIS] Broadcast refreshed shared route "${lastRouteState.routeName}" from disk.`);}}else if(routeFileExists&&lastRouteState?.waypoints?.length){lastRouteState={type:'route-state',routeName:'',waypoints:[],activeWaypointIndex:0,savedAt:new Date().toISOString()};mirrorRouteToShared(lastRouteState);broadcast(lastRouteState);console.log('[AIS] Broadcast shared route clear from disk.');}routeFileExists=existsNow;});
function handleWsConnection(ws){ws.send(JSON.stringify({type:'status',connected:Boolean(serialPort?.isOpen),baudRate:BAUD_RATE,port:serialPort?.path||REQUESTED_PORT||'auto',lastLineAt,lastError}));const disk=loadRouteState();if(disk)lastRouteState=disk;if(lastRouteState){mirrorRouteToShared(lastRouteState);ws.send(JSON.stringify(lastRouteState));}ws.send(JSON.stringify({type:'position-history',entries:positionHistory,sampleCount:positionHistory.length,oldestTimestamp:positionHistory[0]?.timestamp??null,newestTimestamp:positionHistory[positionHistory.length-1]?.timestamp??null}));ws.send(JSON.stringify(getEgcSnapshot()));ws.on('message',raw=>{try{const msg=JSON.parse(String(raw));if(msg.type==='position-history-request'){ws.send(JSON.stringify({type:'position-history',entries:positionHistory,sampleCount:positionHistory.length,oldestTimestamp:positionHistory[0]?.timestamp??null,newestTimestamp:positionHistory[positionHistory.length-1]?.timestamp??null}));}else if(msg.type==='route-state'){lastRouteState=normalizeRouteState(msg);if(lastRouteState.waypoints.length>=2)saveRouteState(lastRouteState);mirrorRouteToShared(lastRouteState);broadcast(lastRouteState);}else if(msg.type==='route-clear'){lastRouteState={type:'route-state',routeName:'',waypoints:[],activeWaypointIndex:0,savedAt:new Date().toISOString()};clearRouteState();mirrorRouteToShared(lastRouteState);broadcast(lastRouteState);}else if(msg.type==='shared-state-get'&&validSharedKey(msg.key)&&typeof msg.requestId==='string'){sendSharedResult(ws,msg.key,msg.requestId);}else if(msg.type==='shared-state-put'&&validSharedKey(msg.key)){const ok=putSharedState(msg.key,msg.value,msg.ttlMs,msg.updatedAt);if(ok)broadcast({type:'shared-state-updated',key:msg.key,updatedAt:sharedState[msg.key]?.updatedAt??null});}else if(msg.type==='shared-state-delete'&&validSharedKey(msg.key)){delete sharedState[msg.key];saveSharedState();broadcast({type:'shared-state-updated',key:msg.key,updatedAt:null});}else if(msg.type==='egc-refresh')scanEgcDirectory(true);}catch{}});}
