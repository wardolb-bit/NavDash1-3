...
export function NavMapMainOverlay() {
...
function IsolatedMainMap() {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      const ownPane = map.createPane("navmap-main-ownship");
      ownPane.style.zIndex = "720";
      ownLayerRef.current = L.layerGroup([], { pane: "navmap-main-ownship" } as any).addTo(map);
      mapRef.current = map;
      ...
      (element as any).__navmapMainInvalidate = invalidate;
    ...
    return () => {
      ...
      if (element?.__navmapMainInvalidate) {
        window.removeEventListener("resize", element.__navmapMainInvalidate);
      }
      if (mapRef.current) {
    ...
    async function updateRoute() {
      ...
      routeMarkersRef.current = route.map((wp, index) =>
        L.circleMarker(points[index] as any, {
          ...
          .bindTooltip(`${wp.id || `WP${index + 1}`} ${wp.name || ""}`.trim())
          .addTo(map),
      );

      try {
        map.fitBounds(L.latLngBounds(points as any), { padding: [35, 35], maxZoom: 10 });
      } catch {}
    }
  ...
  useEffect(() => {
    async function updateOwnShip() {
      ...
      if (Number.isFinite(ownShip.sog) && ownShip.sog > 0.1 && Number.isFinite(ownShip.cog)) {
        ...
        cogVectorRef.current = L.polyline([position, [end.lat, end.lon]], {
          pane: "navmap-main-ownship",
          color: "#22d3ee",
          weight: 2.5,
          opacity: 0.95,
