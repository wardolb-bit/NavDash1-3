...
export default function CelestialConstellationEnhancer(){
  ...
  const constellationLayer=<g aria-label="Constellation overlay">
    {constellations&&C.flatMap(c=>c.segments.map(([a,b],i)=>{const sa=byId.get(a),sb=byId.get(b);if(!sa||!sb)return null;const pa=project(sa),pb=project(sb);if(!pa||!pb)return null;return <line key={`${c.name}-${i}`} x1={pa.x} y1={pa.y} x2={pb.x} y2={pb.y} stroke="#42d3c8" strokeOpacity="0.34" strokeWidth="1.35"/>}))}
  ...
  const controls=<div style={{position:"absolute",right:12,top:12,zIndex:15,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,pointerEvents:"auto"}}>
    <div style={{display:"flex",gap:5,padding:5,border:"1px solid var(--nd-border)",background:"color-mix(in srgb, var(--nd-panel) 92%, transparent)",boxShadow:"0 4px 14px rgba(0,0,0,.18)"}}>
      ...
      <button type="button" onClick={()=>setFieldStars(v=>!v)} style={{height:28,padding:"0 9px",border:"1px solid var(--nd-border)",background:fieldStars?"var(--nd-gold)":"var(--nd-control-bg)",color:fieldStars?"#071019":"var(--nd-control-text)",fontSize:9,fontWeight:900,letterSpacing:".08em"}}>STAR FIELD</button>
    </div>
